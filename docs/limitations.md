...
**Nothing in this repository has been opened in Outlook, Gmail or Apple Mail.** Everything was built against MJML's strict validation and checked by inspecting the generated HTML, and the components' `compat` notes describe how those clients are known to behave, not what was observed here. Expect surprises in:
...
- **The output has never been pasted into a Klaviyo template.** Delivery by manual paste is still an assumption.
- Tags were checked against Klaviyo's help center, not against a Klaviyo account. [Variables](variables.md) marks each entry as verified or not.
