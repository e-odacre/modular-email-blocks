...
- `/components` is the gallery: every component, every variant, and a theme bar that re-renders all of them in another theme.
- `/recipes` shows the campaign recipes with their example content.
- The email links render your real emails, add `?theme=luxury` to any page.
